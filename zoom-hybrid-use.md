...
オーディオに関する対応とビデオに対する対応について順に説明する。
...
このような問題の原因の一つであるエコーを防止するために、Zoomのようなビデオ会議システムを利用する場合には、一つの部屋の中でオーディオを扱う端末を１台に限定することが原則である。詳細については「複数端末を利用する場合の注意（エコーを起こさないために）」(https://kyoto-u.github.io/online-edu/zoom-audio-echo)を参照。
...
接続方式ごとの注意点
-	Bluetooth接続
  -	バッテリー内蔵の製品になるため、授業前の充電を毎回忘れないようにしておく必要がある。
  -	2.4GHz帯の電波を利用するため、Wi-Fiや電子レンジ等との電波干渉により接続が安定しない（音が途切れる、ペアリングが外れる）ことがある。
...
講義室のスクリーンに投影する映像は、講師の端末を直接するのではなく、受講者用として別の端末を用意して、その受信映像をスクリーンに投影する。このようにすることで、遠隔側の受講者が見ている映像を確認しながら授業を進めることができる。

講義室内にパソコンの画面を投影するスクリーン等があれば、パソコンやタブレットのホワイトボードアプリや、パソコンと連携できる電子黒板の表示をビデオ会議システム端末で画面共有したものを表示する形で対面授業にも対応することができる。
...
- 複数の端末が同一の無線LANに接続していない場合であっても、同一あるいは近傍のチャネルを利用することで、利用帯域が減少する。
